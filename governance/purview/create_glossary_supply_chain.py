#!/usr/bin/env python3
"""
Créer le Business Glossary pour le Domain "Supply Chain"
8 termes métier essentiels pour les opérations 3PL
"""
...
def create_term(credential, domain_id, term_data):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"   ✅ Created: {term_data['name']} (ID: {result.get('id')})")
            return result
        else:
...
def main():
    print("\n" + "=" * 80)
    print("  CRÉATION DU BUSINESS GLOSSARY - SUPPLY CHAIN DOMAIN")
    print("=" * 80)
    ...
    for term_data in terms:
        term_result = create_term(credential, domain_id, term_data)
        if term_result:
            created_terms.append(term_result)
    ...
    if created_terms:
        ...
        # Save results
        with open("glossary_terms_supply_chain.json", "w") as f:
            json.dump(created_terms, f, indent=2)
        
        print(f"\n💾 Résultats sauvegardés: glossary_terms_supply_chain.json")
    
    print("\n🌐 Visualiser dans Purview Portal:")
    print(f"   https://web.purview.azure.com/resource/{PURVIEW_ACCOUNT}")
    print(f"   Unified Catalog → Domains → Supply Chain → Glossary")
